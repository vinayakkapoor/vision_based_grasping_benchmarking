...
class EFDDetector(Node):
    def __init__(self):
        super().__init__('efd_detector')
        self.srv = self.create_service(EFDGrasp, '/top_surface_grasp_service/predict', self.handle_get_grasp)
        self.vis_publisher = self.create_publisher(Image, "visualisation_ros2", 1)
    ...
    def get_curve(self, coeffs, locus=(0.0, 0.0), n=300):
    ...
    def compute_tangents_normals(self, xt, yt):
        '''
    ...
    def plot_random_lines(self, xt, yt, tangents, random_indices=None, color='red', scale=0.01):
        '''
    ...
    def find_local_max_min_indices(self, arr):
        '''
    ...
    def have_common_indices(self, arr1, arr2):
        '''
    ...
    def get_extremum_points(self, largest_contour, even_sample=True):
        '''
        Get prospective grasp points from the curve
        '''
        # Fit curve based on EFD
        coeffs = self.elliptic_fourier_descriptors(largest_contour, order=10)
        a0, c0 = self.calculate_dc_coefficients(largest_contour)
        xt, yt = self.get_curve(coeffs, locus=(a0,c0), n=300)
        
        # Compute tangents and normals
        tangents, normals = self.compute_tangents_normals(xt, yt)
        normals_norm = normals / np.linalg.norm(normals, axis=-1, keepdims=True)
    
        # Compute curvature and find extremum points 
        curvature = self.compute_curvature(normals)
        maxima, minima = self.find_local_max_min_indices(curvature)
        maxima_minima = np.concatenate([maxima, minima])
        
        # Filter only concave points
        # concavities = self.find_concave(tangents)
        # concave_indices = np.where(concavities >= 0)[0]    
        # concave_curvature = curvature[concave_indices]
        # concave_maxima_minima = self.have_common_indices(concave_indices, maxima_minima)
    ...
    def get_grasp(self, largest_contour, visualize=False, split=False):
        '''
        ...
        xt, yt, outward_normals, maxima_minima, combinations_list, normals_norm = self.get_extremum_points(outer_contour)
        ...
        if len(grasps) == 0 and len(backup_grasps) == 0:
            grasps.append([[0, 1], 0])
            self.get_logger().error("Grasp Not Found. Tune Parameters")
        if len(grasps) == 0:
        ...
        if visualize:
            ...
            # Publish the image for visualization and logging
            fig = plt.gcf()
            fig.set_dpi(500)
            fig.canvas.draw()
            img = np.array(fig.canvas.renderer.buffer_rgba())
            img_bgr = cv.cvtColor(img, cv.COLOR_RGBA2BGR)
            msg = self.bridge.cv2_to_imgmsg(img_bgr)
            self.vis_publisher.publish(msg)

        return np.array([[x1, y1], [x2, y2]])

    def get_grasp_from_img_file(self, img_file):
        '''
        ...
        grasp = self.get_grasp(largest_contour, visualize=True)
        return grasp

def main(args=None):
    rclpy.init(args=args)
    efd_detector = EFDDetector()
    
    try:
        rclpy.spin(efd_detector)
    except KeyboardInterrupt:
        efd_detector.get_logger().info('Shutting down...')
    finally:
        efd_detector.destroy_node()
        rclpy.shutdown()
