...
class SwinTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=4, embed_dim=96,
                 ...
                 window_size=7, mlp_ratio=4., qkv_bias=True, qk_scale=None, # changed window size form 7 to 9
...
class PyramidFeatures(nn.Module):
    def __init__(self, config, img_size=224, in_channels=3):  
        ...
        unexpected = [
            "patch_embed.proj.weight", "patch_embed.proj.bias", "patch_embed.norm.weight", "patch_embed.norm.bias",
        ...
        # Load ResNet backbone and modify the first layer to accept 22 channels
        resnet = eval(f"torchvision.models.{config.cnn_backbone}(pretrained={config.resnet_pretrained})")
        resnet.conv1 = nn.Conv2d(in_channels, 64, kernel_size=7, stride=2, padding=3, bias=False)  # Updated input channels
        ...
        # Filter out unused weights from the checkpoint
    ...
    def forward(self, x):
        ...
        return [
            torch.cat((sw1_CLS_reshaped, sw1_skipped), dim=1),
