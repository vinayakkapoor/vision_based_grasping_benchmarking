...
class GatedPixelCNNLayer(nn.Module):
    ...
    def forward(self, vstack_input, hstack_input):
        ...
        vstack = self._vstack_1xN(vstack_input)
        vstack = self._vstack_Nx1(vstack)
        vstack = vstack[:, :, :h, :]
...
class GatedPixelCNN(base.AutoregressiveModel):
    ...
    def __init__(
        ...
        self._head = nn.Sequential(
            nn.ReLU(),
            nn.Conv2d(
                in_channels=gated_channels, out_channels=head_channels, kernel_size=1, bias=True
            ),
            nn.ReLU(),
            nn.Conv2d(
                in_channels=head_channels, out_channels=out_channels, kernel_size=1, bias=True
            ),
        )
        self._pos_head = nn.Conv2d(in_channels=out_channels, out_channels=1, kernel_size=1, bias=True)
        self._sin_head = nn.Conv2d(in_channels=out_channels, out_channels=1, kernel_size=1, bias=True)
        self._cos_head = nn.Conv2d(in_channels=out_channels, out_channels=1, kernel_size=1, bias=True)
        self._wid_head = nn.Conv2d(in_channels=out_channels, out_channels=1, kernel_size=1, bias=True)
    ...
    def forward(self, x):
        ...
        out_layer = self._head(skip_connections)
        return self._pos_head(out_layer), self._cos_head(out_layer), self._sin_head(out_layer), self._wid_head(out_layer)
